...
class Build_networks():
    ...
    def MP(self):
        ...
        i = 1
        Actfun_list = ['ReLU','relu','ELU','elu','Identity','I','Softplus','softplus']
        if self.Active_function in Actfun_list:
            ...
            elif self.Active_function == 'Identiy' or 'I':
            ...
            message = str('Your Activation function: '+self.Active_function+' is not allowed.')
            msg.debuginfo(message)
            message = str('Activation function should be one of them: ')
            msg.debuginfo(message)
            message = str(Actfun_list)
            msg.debuginfo(message)
            sys.exit()
...
#        cost = tf.sqrt(tf.reduce_mean(tf.square(logits-self.Y)))
        ...
        while training_switch ==1:
            ...
            with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
                ...
                #Nytrain=Normalize(y_train,Yinfo_data,1) 
...
#                        print('Iteration', '%9d' % (minibatch_iter),'/{}'.format(int(np.shape(NXtrain)[0]/mini_batch_size)))    
                    ...
                    if i==0:
                        ...
                        progress = str('Epoch {:9d}, training cost={:.9f}, Validation cost={:.9f}'.format((i + 1), c, Val_c))
                    ...
                    if i == 9999:
                        ...
                        if judgement > 0.8: # not reduced than 80% of 1st Val_cost
                            training_switch = 0
                        else:
                            training_switch = 1
                ...
                if training_switch == 0:
                    if i % self.SnD_step !=0:
                        progress = str('Epoch {:9d}, training cost={:.9f}, Validation cost={:.9f}'.format((i), c, Val_c))
                        sys.stdout.write('\r'+progress)
